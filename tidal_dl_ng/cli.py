...
dl_group = typer.Typer(
    context_settings={"help_option_names": ["-h", "--help"]},
    add_completion=True,
    help="Download media from TIDAL by URL or favorites collections.",
)
app.add_typer(dl_group, name="dl")
...
@dl_group.command(
    name="url",
    help="Download a track, album, artist, or mix by its TIDAL URL.",
)
def download(
    ctx: typer.Context,
...
def download_fav_tracks(ctx: typer.Context):
    ...
    # Get favorite tracks
    track_urls = [track.share_url for track in cast(Tidal, ctx.obj[CTX_TIDAL]).session.user.favorites.tracks()]
...
def download_fav_artists(ctx: typer.Context):
    ...
    artist_urls = [artist.share_url for artist in cast(Tidal, ctx.obj[CTX_TIDAL]).session.user.favorites.artists()]
...
def download_fav_albums(ctx: typer.Context):
    ...
    # Get favorite albums
    album_urls = [album.share_url for album in cast(Tidal, ctx.obj[CTX_TIDAL]).session.user.favorites.albums()]
    return _download(ctx, album_urls, try_login=False)
...
def download_fav_videos(ctx: typer.Context):
    ...
    # Get favorite videos
    video_urls = [video.share_url for video in cast(Tidal, ctx.obj[CTX_TIDAL]).session.user.favorites.videos()]
    return _download(ctx, video_urls, try_login=False)
