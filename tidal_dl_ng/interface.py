...
def download(uri, typeName: Literal["artist", "album", "track"], folder: Path, fn_print=print) -> Optional[bool]:
    if tidal is None:
...
def search(
    terms: str, typeName: Literal["artist", "album", "track"], extensiveSearch=False, monitored=False, fn_print=print
) -> Optional[list[Artist] | list[Album] | list[Track]]:
    ...
    if not monitored and extensiveSearch is True:
        ...
        logger.info("Checking against human writing speed")
        expectedTime = lastSearch + charCount / 5 + random()  # based on 60 words per minute
        logger.debug(f"{time()} expecting {expectedTime}")
...
def searchOne(
    ...
    verifyMeta: Optional[
        Callable[[Any, Artist | Album | Track, Literal["artist", "album", "track"]], bool]
    ] = None,
